...
import h5py, glob
import os
from XFDing import read_images
...
def gen_train_batch_bg_from_folder(dsfn, mb_size, in_depth, img_size):
    X = read_images(os.path.join(dsfn,"train_ns")).astype(np.float32)
    Y = read_images(os.path.join(dsfn,"train_gt")).astype(np.float32)
...
def get1batch4test_from_folder(dsfn, in_depth):
    X = read_images(os.path.join(dsfn,"test_ns")).astype(np.float32)
    Y = read_images(os.path.join(dsfn,"test_gt")).astype(np.float32)
